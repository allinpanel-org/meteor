 ...
 */
import type Repository from "@allinpanel-ag/meteor-admin-sdk/es/data/repository";
import Criteria from "@allinpanel-ag/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@allinpanel-ag/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@allinpanel-ag/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@allinpanel-ag/meteor-admin-sdk");
